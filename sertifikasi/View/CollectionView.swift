            ...
            ScrollView {
                VStack{
                    ForEach((0..<viewModel.books.books.count / 2 + 1), id: \.self) { i in
                        HStack{
                            BookCard(bookID: viewModel.books.books[i * 2].id!, title: viewModel.books.books[i * 2].wrappedTitle)
                            
                            if viewModel.books.books.count > i * 2 + 1 {
                                BookCard(bookID: viewModel.books.books[i * 2 + 1].id!, title: viewModel.books.books[i * 2 + 1].wrappedTitle)
                            } else {
                                Spacer()
                            }
                        }
